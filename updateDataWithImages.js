...
// We want to replace the `image: null,` line inside pokemonBoosterBoxes with unique real images.
// Or we can just rebuild the array again.
...
const newBoosterBoxes = csvData.map((item, index) => {
  const img = imageUrls[index % imageUrls.length]; // guarantee unique/rotating image
  return `  {
...
if (startIndex !== -1) {
  ...
  if (endMatch) {
    ...
    console.log("Updated data.js with real scraped images!");
  }
}
