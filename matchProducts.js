...
function findShopifyProduct(name) {
  // Extract set code like (M6) or (SV11W)
  const codeMatch = name.match(/\(([A-Za-z0-9]+)）?\)?/);
  const code = codeMatch ? codeMatch[1].toLowerCase() : null;
  
  // Clean name
  let cleanName = name.replace(/\(.*?\)/, '').replace(/（.*?）/, '').trim().toLowerCase();
  
  for (let p of data.products) {
    let pTitle = p.title.toLowerCase();
    
    // Exact code match
    if (code && pTitle.includes(`【${code}】`)) {
      return p;
    }
    
    // Keyword match
    let words = cleanName.split(' ');
    if (words.length > 0 && words.every(w => pTitle.includes(w))) {
      return p;
    }
  }
...
// Fallback images from data.js just in case
...
const newBoosterBoxes = csvData.map((item, index) => {
  const shopifyProd = findShopifyProduct(item.name);
  let finalName = item.name;
  let finalImage = "null";
  
  if (shopifyProd) {
    finalName = shopifyProd.title;
    if (shopifyProd.images && shopifyProd.images.length > 0) {
      finalImage = `"${shopifyProd.images[0].src}"`;
    }
  } else {
     // If not found, use a placeholder or something 
     // Wait, previously they were 404ing, so we'll just set it to null
     finalName = `【${item.name}】TCG BOX & CASE〔Factory Sealed〕`;
  }
...
if (startIndex !== -1) {
  ...
  if (endMatch) {
    ...
    console.log("Updated data.js with matched Shopify titles and images!");
  }
}
